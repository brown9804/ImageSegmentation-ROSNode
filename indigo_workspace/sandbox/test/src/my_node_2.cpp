...
//***************FIN PASO (2)****************
{
    ...
    printf("****************************************************************************\n");
    printf("** ROSS node#2                                                            **\n");
    printf("** YUV, linea, circulo en imagenes capturas en tiempo real                **\n");
...
void geoLeerParametrosDeControlDeArchivoDeTexto()
{
    ...
    char nombreDeArchivo[256]="current_control_parameters(my_node_2).txt";
    archivo = fopen(nombreDeArchivo, "r");
    if (!archivo) {
        printf("No se pudo abrir el archivo: current_control_parameters(my_node_2).txt\n");
        exit(1);
    }
